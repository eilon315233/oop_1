...
public abstract class ConcretePiece implements Piece{
    ...
    public int getDistance(){
        int ans=0;
        List<Position> posHis=this.CleanMinusOne();
        for(int i=0;i<posHis.size()-1;i++){
            int ax=posHis.get(i).getX(), ay=posHis.get(i).getY();
            int bx=posHis.get(i+1).getX(), by=posHis.get(i+1).getY();
            if(ax==bx)
                ans+=Math.abs(ay-by);
            else
                ans+=Math.abs(ax-bx);
